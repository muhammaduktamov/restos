...
function normalizeProgress(value) {
  ...
  return Math.max(0, Math.min(Math.round(value), 150));
}

function getTargets(settings = {}) {
  return {
    avgGuestCheck: parseNumber(settings.avg_guest_check_target) || 390000,
    avgTableCheck: parseNumber(settings.avg_table_check_target) || 730000,
    marginPercent: parseNumber(settings.margin_percent_target) || 32,
    dessertPercent: parseNumber(settings.dessert_percent_target) || 100
  };
}

function computeDerivedMetrics(entry, targets) {
  const avgGuestProgress = entry.avgGuestCheck > 0
    ? normalizeProgress((entry.avgGuestCheck / targets.avgGuestCheck) * 100)
    : 0;

  const avgTableProgress = entry.avgTableCheck > 0
    ? normalizeProgress((entry.avgTableCheck / targets.avgTableCheck) * 100)
    : 0;

  const marginPercent = entry.totalDishes > 0
    ? Number(((entry.marginalDishes / entry.totalDishes) * 100).toFixed(1))
    : 0;

  const marginProgress = marginPercent > 0
    ? normalizeProgress((marginPercent / targets.marginPercent) * 100)
    : 0;

  const dessertPercent = entry.tables > 0
    ? Number(((entry.desserts / entry.tables) * 100).toFixed(1))
    : 0;

  const dessertProgress = dessertPercent > 0
    ? normalizeProgress((dessertPercent / targets.dessertPercent) * 100)
    : 0;

  const calculatedKpi = Number((
  ...
  return {
    ...
    dessertProgress,
    calculatedKpi,
    kpi: entry.kpi > 0 ? entry.kpi : calculatedKpi
  };
}
...
async function loadRawData() {
  ...
  const [weeklyRes, settingsRes] = await Promise.all([
    sheets.spreadsheets.values.get({
      ...
      range: `${WEEKLY_SHEET_NAME}!A1:L1000`
    }),
    sheets.spreadsheets.values.get({
      spreadsheetId: SPREADSHEET_ID,
  ...
  const entries = dataRows.map((row) => {
    ...
    return {
      ...
      totalDishes:
        parseNumber(get('total_dishes')) ||
        parseNumber(get('all_dishes')) ||
        parseNumber(get('total_positions')) ||
        parseNumber(get('total_items')),
      tables:
        parseNumber(get('tables')) ||
        parseNumber(get('table_count')),
...
function buildWeeklyResponse(entries, settings, requestedWeek) {
  ...
  const waiters = currentEntries
    .map((entry) => {
      ...
      const derived = computeDerivedMetrics(entry, targets);
      ...
      return {
        ...
        totalDishes: entry.totalDishes,
        tables: entry.tables,

        marginPercent: derived.marginPercent,
        dessertPercent: derived.dessertPercent,

        avgGuestProgress: derived.avgGuestProgress,
        avgTableProgress: derived.avgTableProgress,
        marginProgress: derived.marginProgress,
        dessertProgress: derived.dessertProgress,
        ...
        trendKpi: percentChange(derived.kpi, previous?.kpi || 0)
      };
    })
  ...
  const summary = {
    ...
    avgGuestCheck: waiters.length
      ? Math.round(waiters.reduce((sum, w) => sum + w.avgGuestCheck, 0) / waiters.length)
      : 0,
    avgTableCheck: waiters.length
      ? Math.round(waiters.reduce((sum, w) => sum + w.avgTableCheck, 0) / waiters.length)
      : 0,
    totalDesserts: waiters.reduce((sum, w) => sum + w.desserts, 0),
    avgKpi: waiters.length
      ? Number((waiters.reduce((sum, w) => sum + w.kpi, 0) / waiters.length).toFixed(1))
      : 0
  };
...
function buildMonthlyResponse(entries, settings, requestedMonth) {
  const targets = getTargets(settings);
  ...
  for (const entry of monthEntries) {
    ...
    item.totalDishes += entry.totalDishes;
    item.tables += entry.tables;
  ...
  const waiters = [...waiterMap.values()]
    .map((item) => {
      const avgGuestCheck = item.weeksCount
        ? Math.round(item.avgGuestCheckSum / item.weeksCount)
        : 0;

      const avgTableCheck = item.weeksCount
        ? Math.round(item.avgTableCheckSum / item.weeksCount)
        : 0;
...
app.get('/api/dashboard', async (req, res) => {
  try {
    ...
    if (period === 'monthly') {
      return res.json(buildMonthlyResponse(entries, settings, String(req.query.month || '')));
    }
